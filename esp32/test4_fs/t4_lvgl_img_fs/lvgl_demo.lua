...
function img_png()
    ...
    lvgl.img_set_src(img, "/luadb/"..img_file_name)-- 设置图片路径
...
function lvgl_img()
    ...
    -- log.info("load img")
    -- image_object = image.load("/luadb/test3.bmp")
    -- log.info("load img")

    -- local scr = lvgl.obj_create()
    img_label = lvgl.obj_create(nil,nil)
    -- img_label = lvgl.label_create(nil,nil)
    local img = lvgl.img_create(img_label,nil)			--
    lvgl.img_set_src(img, "/luadb/test3.bmp")		--设置图片来源
    lvgl.obj_set_size(img,128,160)
    lvgl.obj_set_pos(img,0,0)

    -- lvgl.obj_align(img, , lvgl.ALIGN_CENTER, 0, 0) 	--图片居中
    log.info("scr_load",lvgl.scr_load(img_label))
    -- lvgl.draw_img(img)
