...
function ws_send_t1(str_send)
    temp_data_send="abcaaaa"
    data_send_len=string.len(temp_data_send)
    mask={}

    -- a=WS_FIN + WS_OPCODE_TEXT
    -- a=tostring(a)
    -- m=string.format("%02X",a)
    -- print("m",m)
    mask[1]=string.format("%02X",tostring(WS_FIN + WS_OPCODE_TEXT))
    -- print(mask[1])
    mask[2]=string.format("%02X",tostring(WS_MASK + data_send_len))
    -- print(mask[2])

    mask[3]=temp_data_send
    mask[4]="ada7e68"--masking-key
    
    data_send=table.concat(mask)
    
    print(data_send)
    
    local data_send_len = socket.send(websocket_client, string.dump(data_send))
    log.info("socket", ">sendlen", data_send_len)
    print(">>>>>>>>>>>>>>>>>>>>>>")
    print(data_send)
    print(">>>>>>>>>>>>>>>>>>>>>>")
...
function ws_connect()
    ...
    set_ws_head()
    len = socket.send(websocket_client, msg_send)
