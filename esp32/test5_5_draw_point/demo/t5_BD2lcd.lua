...
scale_max = 0.00738400/2 --BD最大偏移值，2scale_max 映射 128        这是图像精度，变小则图像在lcd上绘制大
LCD_max = 64     --64/128
...
-- 存储画点数据
LCD_list = {}
xy_list_BD = { {120.541275,31.31075200},
...
function Ref_cur_bias(x,y,lcdmax,scalemax)
    ...
    if type(x) == "nil" or type(y) == "nil" or  type(lcdmax) == "nil" or type(scalemax) == "nil" then
        ...
        Ref_bias_x = math.floor(cur_BD_bias[1]*LCD_max/(2*scale_max))   -- + P00_Ref[1]
        Ref_bias_y = math.floor(cur_BD_bias[2]*LCD_max/(2*scale_max))   -- + P00_Ref[2]
    else
        Ref_bias_x = math.floor(x*lcdmax/(2*scalemax))   -- + P00_Ref[1]
        Ref_bias_y = math.floor(x*lcdmax/(2*scalemax))   -- + P00_Ref[2]
    end
